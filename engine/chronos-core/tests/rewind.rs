...
fn world_data() -> (Vec<(&'static str, &'static str)>, Vec<(&'static str, &'static str)>) {
    let rooms = vec![
        ("dim_corridor.json", r#"{
            "id": "dim_corridor",
            "name": "Dim Corridor",
...
fn plague_repo() -> StaticRepository {
    let (rooms, items) = world_data();
    let classes = vec![
        ("plague_knight.json", r#"{ "id": "plague_knight", "name": "Plague Knight", "description": "t",
        ...
        ("goblin.json", r#"{ "id": "goblin", "name": "Goblin", "description": "g",
...
fn tactic_repo() -> StaticRepository {
    let (rooms, items) = world_data();
    let classes = vec![
        ("fighter.json", r#"{ "id": "fighter", "name": "Fighter", "description": "t",
            "base_stats": { "hp": 120, "attack": 9, "defense": 8, "intelligence": 2 } }"#),
        ("goblin.json", r#"{ "id": "goblin", "name": "Goblin", "description": "g",
...
fn rewind_restores_picked_up_item_to_room() {
    ...
    assert_eq!(engine.current_tick(), 1, "rewind must land on the target tick");
    assert_eq!(engine.max_tick(), 2, "rewind must not append to the event log");

    // The key should be back in the room (inventory empty).
    let snap = engine.snapshot();
    assert!(snap.inventory_ids.is_empty(), "key should not be held after rewind to tick 1");
}
...
fn spawn_character_replays_under_rewind() {
    ...
    assert!(engine.snapshot().player_character.is_none(), "no character before spawn");

    // tick 1: become a Fighter.
    let res = engine.process_command("become fighter Aragorn");
    assert!(res.success, "spawn should succeed");
    let sheet = engine.snapshot().player_character.expect("character after spawn");
    ...
    assert!(engine.snapshot().player_character.is_some(), "character persists after moving");

    // Rewind to tick 0 — before creation. The character must be gone.
    engine.rewind_to_tick(0);
    assert!(engine.snapshot().player_character.is_none(), "rewind past spawn un-makes the character");

    // Replay forward to tick 1 — the character must come back, identical.
    engine.rewind_to_tick(1);
    let restored = engine.snapshot().player_character.expect("character restored by replay");
...
fn unknown_class_is_rejected_without_advancing_state() {
    ...
    assert!(engine.snapshot().player_character.is_none(), "no character imprinted on failure");
}
...
fn combat_replays_identically_under_rewind() {
    ...
    assert_eq!(replay.enemies[0].hp, enemy_hp_t2, "enemy HP must replay identically");
    assert_eq!(
        replay.player_character.as_ref().unwrap().hp, player_hp_t2,
...
fn poison_ticks_and_replays_under_rewind() {
    ...
    assert_eq!(replay3.enemies[0].hp, 28, "rewind tick 3: poison damage replayed identically");
...
fn poison_expires_and_never_reapplies_under_rewind() {
    ...
    assert_eq!(snap6.enemies[0].hp, 24, "no more poison damage after expiration");
    ...
    let snap4 = engine.snapshot();
    assert_eq!(snap4.enemies[0].hp, 26, "rewind to tick 4: more poison damage visible");
}
...
fn describe_current_does_not_mutate_history() {
    ...
    assert_eq!(engine.current_tick(), tick_before, "describe_current must not advance the tick");
    assert_eq!(engine.max_tick(), max_before, "describe_current must not append to the log");
    assert!(peek.narrative.contains("Dim Corridor"), "peek should describe the rewound room");
    // Calling it repeatedly must be idempotent — no drift, no borrow corruption.
    let peek2 = engine.describe_current();
    assert_eq!(engine.max_tick(), max_before, "repeated peeks must not grow history");
    assert_eq!(peek.narrative, peek2.narrative, "peeks must be deterministic");
}
...
fn burn_ticks_and_replays_under_rewind() {
    ...
    let snap5 = engine.snapshot();
    assert_eq!(snap5.enemies[0].hp, 24, "burn dealt 6 total damage over 2 ticks");
    ...
    engine.rewind_to_tick(3);
    assert_eq!(engine.snapshot().enemies[0].hp, 27, "rewind tick 3: one burn tick");
...
fn bleed_via_ability_replays_under_rewind() {
    ...
    assert!(snap2.enemies[0].active_effects.contains(&"Bleed".to_string()), "bleed applied");
    ...
    assert_eq!(engine.snapshot().enemies[0].hp, 76, "4 bleed ticks: -8 total");

    // Rewind to tick 3 — one bleed tick, goblin should be at 82.
    engine.rewind_to_tick(3);
    assert_eq!(engine.snapshot().enemies[0].hp, 82, "rewind tick 3: one bleed tick");
...
fn level_up_replays_under_rewind() {
    ...
    assert_eq!(snap3.attack, atk_before_levelup + 1, "ATK boosted on level-up");
    ...
    assert_eq!(reverted.attack, atk_before_levelup, "rewind undoes stat boost");
...
fn enemy_tactic_heavy_attack_fires_after_first_hit() {
    ...
    assert_eq!(snap_before.enemies[0].hp, goblin_max, "goblin starts at full HP");
    ...
    let snap2 = engine.snapshot();
    assert!(snap2.enemies[0].hp < goblin_max, "goblin took damage from player hit");

    // Rewind to tick 1 — goblin should be back at full HP, player back at imprint state.
    engine.rewind_to_tick(1);
    let reverted = engine.snapshot();
    assert_eq!(reverted.enemies[0].hp, goblin_max, "rewind restores goblin to full HP");
...
fn healing_ability_restores_hp_and_replays_under_rewind() {
    ...
    assert!(result.narrative.contains("95/95"), "full-HP heal caps at max: {}", result.narrative);
    ...
    let classes2 = vec![
        ("medica.json", r#"{ "id": "medica", "name": "Medica", "description": "t",
    ...
    let manifest2 = r#"{ "start_room_id": "dim_corridor",
        "encounters": [ { "class_id": "goblin", "room_id": "dim_corridor" } ] }"#;
    let repo2 = StaticRepository::from_json_pairs(&rooms2, &items2, &classes2, Some(manifest2)).unwrap();
    ...
    assert!(snap_after_hit.hp < 50, "goblin hit us: hp={}", snap_after_hit.hp);
    let hp_after_hit = snap_after_hit.hp;

    // tick 3: heal with healing_touch (20 HP)
    let heal_result = e2.process_command("healing touch");
    assert!(heal_result.success, "heal should succeed: {}", heal_result.narrative);
    let snap3 = e2.snapshot().player_character.unwrap();
    let expected_hp = (hp_after_hit + 20).min(50);
    assert_eq!(snap3.hp, expected_hp, "healing_touch restored 20 HP: {}", heal_result.narrative);
...
fn multi_hit_ability_deals_multiple_hits_and_replays_under_rewind() {
    ...
    assert!(result.success, "whirlwind slash failed: {}", result.narrative);
    assert!(result.narrative.contains("3 hits"), "narrative should mention 3 hits: {}", result.narrative);
    ...
    assert!(damage_dealt >= 33 && damage_dealt <= 39,
    ...
    engine.rewind_to_tick(1);
    assert_eq!(engine.snapshot().enemies[0].hp, goblin_hp_before, "rewind restores goblin HP");
...
fn defense_buff_applies_and_expires_and_replays_under_rewind() {
    ...
    let (rooms, items) = world_data();
    let classes = vec![("fighter.json", r#"{
    ...
    assert!(result.success, "shield fortify failed: {}", result.narrative);
    assert!(result.narrative.contains("Shield Fortify") || result.narrative.contains("defense"),
        "should mention shield fortify or defense: {}", result.narrative);
    ...
    assert_eq!(engine.snapshot().player_character.unwrap().defense, 13, "buff still active tick 3");

    // tick 4: second and final buff tick
    engine.process_command("look");  // tick 4
    assert_eq!(engine.snapshot().player_character.unwrap().defense, 13, "buff still active tick 4");

    // tick 5: buff expires (end_tick = applied_at + duration = 2 + 2 = 4, so expired by tick 5)
    engine.process_command("look");  // tick 5
    assert_eq!(engine.snapshot().player_character.unwrap().defense, 8, "buff expired, back to base 8");

    // Rewind to tick 2 — buff should be active again.
    engine.rewind_to_tick(2);
    assert_eq!(engine.snapshot().player_character.unwrap().defense, 13, "rewind tick 2: buff reapplied");

    // Rewind to tick 1 — before the ability, base DEF restored.
    engine.rewind_to_tick(1);
    assert_eq!(engine.snapshot().player_character.unwrap().defense, 8, "rewind tick 1: base DEF restored");
...
fn use_item_heal_restores_hp_and_replays_under_rewind() {
    ...
    let rooms = vec![
        ("arena.json", r#"{ "id": "arena", "name": "Arena", "description": ".", "exits": {} }"#),
    ];
    let items = vec![
        ("health_potion.json", r#"{
    ...
    assert!(result.narrative.contains("restoring"), "narrative should mention restoring HP");

    let hp_healed = engine.snapshot().player_character.unwrap().hp;
    assert!(hp_healed > hp_after_combat, "HP should increase after potion");
    ...
    assert_eq!(rewound.player_character.unwrap().hp, hp_after_combat, "rewind restores pre-heal HP");
    assert!(!rewound.inventory_ids.is_empty(), "potion back in inventory after rewind");
...
fn use_item_stat_boost_applies_and_replays_under_rewind() {
    ...
    let rooms = vec![
        ("forge.json", r#"{ "id": "forge", "name": "Forge", "description": ".", "exits": {} }"#),
    ];
    let items = vec![
        ("strength_elixir.json", r#"{
    ...
    assert!(result.narrative.contains("ATK increased"), "narrative should mention ATK increase");
    ...
    engine.rewind_to_tick(2);
    let rewound = engine.snapshot();
    assert_eq!(rewound.player_character.unwrap().attack, 9, "rewind restores ATK to 9");
    assert!(!rewound.inventory_ids.is_empty(), "elixir back in inventory");
...
fn gold_awarded_on_enemy_kill_and_replays_under_rewind() {
    ...
    let rooms = vec![
        ("pit.json", r#"{ "id": "pit", "name": "Pit", "description": ".", "exits": {} }"#),
    ];
    let items = vec![];
    let classes = vec![
        ("fighter.json", r#"{
    ...
    assert!(result.narrative.contains("+5 scraps"), "kill narrative should mention scraps: {}", result.narrative);
...
fn buy_item_from_vendor_deducts_gold_and_adds_to_inventory() {
    ...
    let rooms = vec![
        ("inn.json", r#"{ "id": "inn", "name": "The Inn", "description": ".", "exits": {} }"#),
    ];
    let items = vec![
        ("health_potion.json", r#"{
            "id": "health_potion", "name": "Health Potion",
    ...
    let repo = StaticRepository::from_json_pairs_with_npcs(
        &rooms, &items, &classes, &npcs, Some(manifest)
    ).unwrap();
    ...
    assert!(!broke_result.success, "should fail with 0 gold: {}", broke_result.narrative);
    assert!(broke_result.narrative.contains("only have 0"), "narrative should say 0 gold");
...
fn shop_listing_shows_vendor_wares() {
    ...
    assert!(result.success, "shop command should succeed: {}", result.narrative);
    assert!(result.narrative.contains("Health Potion"), "should list health potion: {}", result.narrative);
    assert!(result.narrative.contains("15 gold"), "should show price: {}", result.narrative);
    // Can't afford it, so no "Buy" context action should be active
    assert!(result.context_actions.iter().any(|a| a.label.contains("can't afford")),
        "should show can't-afford label: {:?}", result.context_actions);
}
...
fn quest_accept_and_kill_count_completes_and_replays() {
    ...
    let repo = StaticRepository::from_json_pairs_full(
        &rooms, &items, &classes, &npcs, &quests, Some(manifest)
    ).unwrap();
    ...
    assert!(accept.narrative.contains("Goblin Hunt"), "accept narrative: {}", accept.narrative);

    // Kill first goblin — quest progresses (1/2)
    let kill1 = engine.process_command("attack");  // tick 3
    assert!(kill1.narrative.contains("1/2") || kill1.narrative.contains("[Quest"),
        "first kill should show progress: {}", kill1.narrative);
    ...
    assert!(kill2.narrative.contains("Objective complete") || kill2.narrative.contains("Goblin Hunt"),
        "second kill should show quest objective met: {}", kill2.narrative);

    // Turn in the quest to collect reward (giver is in same room)
    let turn_in = engine.process_command("turn in goblin_hunt");  // tick 5
    assert!(turn_in.success, "turn in should succeed: {}", turn_in.narrative);

    let snap5 = engine.snapshot().player_character.unwrap();
    assert!(snap5.gold > gold_before, "quest reward gold should be added after turn-in: {} > {}", snap5.gold, gold_before);

    // Check quest log shows completed
    let log = engine.process_command("quests");
    assert!(log.narrative.contains("COMPLETED"), "quest log should show completed: {}", log.narrative);

    // Rewind to tick 3 (after first kill, before second) — quest should be 1/2
    engine.rewind_to_tick(3);
    let reverted_log = engine.process_command("quests");
    assert!(!reverted_log.narrative.contains("COMPLETED"),
        "after rewind, quest should not be complete: {}", reverted_log.narrative);

    // Gold should be back to first-kill-only level
    let reverted_snap = engine.snapshot().player_character.unwrap();
    assert!(reverted_snap.gold <= gold_before,
        "after rewind, quest reward gold should be gone: {} <= {}", reverted_snap.gold, gold_before);
...
fn victory_fires_when_all_quests_complete_and_replays() {
    ...
    let npcs = vec![
        ("captain.json", r#"{ "id": "captain", "name": "Captain", "greeting": "Fight!" }"#),
    ];
    let quests = vec![
        ("kill_one.json", r#"{
    ...
    assert!(result.narrative.contains("Objective complete") || result.narrative.contains("First Blood"),
        "kill should show quest objective met: {}", result.narrative);

    // Turn in to trigger victory
    let turn_in = engine.process_command("turn in kill_one");  // tick 4
    assert!(turn_in.narrative.contains("VICTORY") || turn_in.narrative.contains("Quest complete"),
        "victory/completion should fire on turn in: {}", turn_in.narrative);
    ...
    assert!(turn_in2.narrative.contains("VICTORY") || turn_in2.narrative.contains("Quest complete"),
        "victory should re-fire after replay: {}", turn_in2.narrative);
}
...
fn equipment_pickup_grants_stat_bonus_and_replays_under_rewind() {
    ...
    // Rewinding undoes it.
    let rooms = vec![
        ("smithy.json", r#"{ "id": "smithy", "name": "Smithy", "description": ".", "exits": {} }"#),
    ];
    let items = vec![
        ("iron_sword.json", r#"{
    ...
    assert!(result.narrative.contains("+4 ATK"), "pickup note should mention +4 ATK: {}", result.narrative);
...
fn equipment_drop_removes_stat_bonus() {
    ...
    assert!(drop_result.success, "drop failed: {}", drop_result.narrative);
    assert!(drop_result.narrative.contains("4 ATK"), "drop note should mention ATK removal: {}", drop_result.narrative);
...
fn inventory_shows_equipment_annotation() {
    ...
    assert!(inv.narrative.contains("[+4 ATK]"),
        "inventory should annotate equipment: {}", inv.narrative);
}
...
fn examine_item_in_room_returns_description() {
    ...
    assert!(result.success, "examine should succeed: {}", result.narrative);
    assert!(result.narrative.contains("A well-balanced blade."),
        "description missing: {}", result.narrative);
    assert!(result.narrative.contains("Iron Sword"),
        "item name missing: {}", result.narrative);

    // Fuzzy name match
    let result2 = engine.process_command("examine sword");
    assert!(result2.success, "fuzzy examine should succeed: {}", result2.narrative);
    assert!(result2.narrative.contains("A well-balanced blade."),
        "fuzzy description missing: {}", result2.narrative);
...
fn examine_item_in_inventory_returns_description() {
    ...
    let result = engine.process_command("examine iron_sword");
    assert!(result.success, "examine held item should succeed: {}", result.narrative);
    assert!(result.narrative.contains("A well-balanced blade."),
        "held item description missing: {}", result.narrative);
}
...
fn kill_context_action_shows_attack_when_enemies_remain() {
    ...
    assert!(kill1.narrative.contains("collapses, slain"), "expected kill narrative: {}", kill1.narrative);
    assert!(
    ...
    assert!(kill2.narrative.contains("collapses, slain"), "expected kill narrative: {}", kill2.narrative);
    assert!(
    ...
    engine.rewind_to_tick(1);
    let snap = engine.snapshot();
    let goblins_in_arena: Vec<_> = snap.enemies.iter().filter(|e| e.room_id == "arena").collect();
    assert_eq!(goblins_in_arena.len(), 2, "rewind should restore both goblins");
}
...
fn rest_at_inn_restores_hp_and_deducts_gold_and_replays() {
    ...
    assert!(hp_damaged < 100, "goblin should have dealt damage: hp={hp_damaged}");

    // Can't rest with 0 gold
    let broke_rest = engine.process_command("rest");  // tick 3
    assert!(!broke_rest.success, "should fail with no gold: {}", broke_rest.narrative);
    assert!(broke_rest.narrative.contains("5 gold"), "should mention cost: {}", broke_rest.narrative);
    ...
    let repo2 = StaticRepository::from_json_pairs_with_npcs(
        &rooms2, &[], &classes2, &npcs2, Some(manifest2)
    ).unwrap();
    ...
    assert!(gold_after_fight >= 5, "should have enough gold to rest: {gold_after_fight}");
    ...
    assert!(rest.narrative.contains("restored"), "should mention restore: {}", rest.narrative);

    let gold_after_rest = e2.snapshot().player_character.unwrap().gold;
    assert_eq!(gold_after_rest, gold_after_fight - 5, "5 gold should be deducted");
