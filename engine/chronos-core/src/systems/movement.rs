...
pub fn process_move(world: &mut World, repo: &StaticRepository, direction: &str) -> MoveResult {
    ...
    world.entity_mut(player_entity)
        .insert(Position { room_id: target_room_id.clone() });
    ...
    if !enemies_here.is_empty() {
        context_actions.insert(0, ContextAction { label: "Attack".to_string(), command: "attack".to_string() });
    }
    ...
    MoveResult { success: true, narrative, context_actions }
}
...
fn talk_npc_actions(room_id: &str, repo: &StaticRepository) -> Vec<ContextAction> {
    repo.npcs_in_room(room_id)
        .iter()
        .filter_map(|id| repo.npc(id).ok().map(|n| ContextAction {
...
fn takeable_item_actions(world: &mut World, room_id: &str, repo: &StaticRepository) -> Vec<ContextAction> {
    let mut query = world.query::<(&Position, &ItemBlueprint)>();
    query.iter(world)
        .filter(|(pos, _)| pos.room_id == room_id)
...
fn error_result(msg: &str) -> MoveResult {
    MoveResult { success: false, narrative: msg.to_string(), context_actions: vec![] }
}
